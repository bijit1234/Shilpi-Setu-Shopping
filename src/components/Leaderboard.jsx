...
export default function Leaderboard({ open = true, onClose, embedMode = false }) {
    ...
    if (!open)
        return null;
    const content = (<div className={(embedMode
      ...
      `}>
        ...
        {!embedMode && onClose && (<button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full hover:bg-[var(--muted)]/10 transition-colors text-[var(--muted)] hover:text-[var(--text)]">
            <X className="w-5 h-5"/>
          </button>)}
        ...
        <div className="flex flex-col items-center text-center mb-8 relative z-10">
          ...
          <p className="text-[var(--muted)]">
            {t('leaderboard.desc', { defaultValue: 'Top contributors to the Shilpi Setu community' })}
          </p>
        </div>

        {loading ? (<div className="space-y-4 animate-pulse">
            {[1, 2, 3].map((i) => (<div key={i} className="h-20 bg-[var(--muted)]/10 rounded-xl w-full"></div>))}
            ...
            {leaders.map((leader, index) => {
                ...
                return (<Link href={`/profile/${leader.user_id}`} key={leader.user_id} className={`
                  ...
                  <div className="ml-4 flex-shrink-0 relative">
                    <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[var(--heritage-gold)] bg-[var(--bg-3)]">
                      {leader.profile_image ? (<img src={leader.profile_image} alt={leader.name || 'User'} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[var(--heritage-gold)] to-[var(--heritage-red)] text-white font-bold text-lg">
    ...
    if (embedMode)
        return content;
    return (<div className="fixed inset-0 bg-black/60 z-[60] backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
