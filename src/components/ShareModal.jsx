...
export const ShareModal = ({ open, onClose, shareUrl, getShareTitle, t, handleCopyLink, handleNativeShare, handleShareWhatsApp, handleShareTwitter, handleShareFacebook }) => {
    if (!open)
        return null;
    return (<div className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-[2px] flex items-center justify-center z-50 p-4" // Overlay: theme-aware
     onClick={onClose}>
      <div className="card-glass p-4 md:p-6 max-w-md w-full animate-slide-in-up rounded-3xl shadow-2xl border border-[var(--border)] bg-[var(--bg-modal),_theme(colors.white/80)] dark:bg-neutral-900/90" // Card: theme-aware and fallback
     style={{ backgroundColor: 'var(--bg-modal,rgba(255,255,255,0.85))' }} onClick={e => e.stopPropagation()}>
