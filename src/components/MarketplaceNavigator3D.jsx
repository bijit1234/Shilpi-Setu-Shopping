...
import { initMarketplaceScene } from '../utils/marketplaceScene';
import { supabase } from '../lib/supabase';
import { useTheme } from '../components/ThemeProvider';
export default function MarketplaceNavigator3D({ sellers, onProductClick, focusSellerId }) {
    ...
    const [stallCustomizations, setStallCustomizations] = useState({});
    useEffect(() => {
            ...
            if (focusSellerId) {
                ...
                if (stall && stall.gotoButton) {
                    current.stalls.forEach((s) => {
                        const g = s.group;
                        if (g && typeof g.setFocused === 'function')
                            g.setFocused(false);
                    });
                    const group = stall.group;
    ...
    const onClick = (e) => {
        ...
        const meshes = [
            ...
            ...current.stalls.map((s) => s.gotoButton).filter((btn) => !!btn),
            // Add carousel arrows if present
            ...current.stalls.flatMap((s) => {
                const group = s.group;
                const arrows = [];
        ...
        if (pick.length > 0) {
            ...
            const id = hit.userData?.productId;
            if (id) {
                onProductClick(id);
                return;
            }
            ...
            if (action === 'goto-stall') {
                const sellerId = hit.userData?.sellerId;
    ...
    const onDoubleClick = (e) => {
        ...
        const groundMeshes = current.scene.children.filter((obj) => obj.name === 'ground' || (obj.userData && obj.userData.ground));
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            const current = sceneRef.current;
            if (!current || !current.controls || !current.camera)
                return;
            const controls = current.controls;
            const cam = current.camera;
    ...
    return (<div className={`relative w-full max-w-6xl h-[70vh] rounded-2xl overflow-hidden shadow-2xl border border-yellow-400/20 ${containerBg}`}>
      <canvas ref={canvasRef} className="w-full h-full block" onClick={onClick} onDoubleClick={onDoubleClick}/>
      ...
      <div className={`absolute left-0 right-0 flex flex-col items-center z-10 ` +
        ...
        <div className="flex justify-center gap-4">
          <button className="px-4 py-2 rounded bg-yellow-500 text-white font-bold disabled:opacity-50" onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}>
            Prev
          </button>
          <span className="px-3 py-2 bg-white/80 rounded text-yellow-900 font-semibold shadow">Page {page + 1} / {totalPages}</span>
          <button className="px-4 py-2 rounded bg-yellow-500 text-white font-bold disabled:opacity-50" onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1}>
            Next
          </button>
        </div>
      </div>
      {/* Controls overlay: hidden on mobile, smaller on laptop/desktop */}
      <div className={`pointer-events-none absolute top-2 right-2 ${overlayBg} text-[0.65rem] px-2 py-0.5 rounded-lg shadow-lg font-semibold backdrop-blur-md hidden sm:flex sm:items-center sm:gap-1 md:text-[0.7rem] md:px-2 md:py-1 md:rounded-xl lg:text-[0.8rem] lg:px-3 lg:py-1 lg:rounded-xl`}>
      ...
      {isMobile && (<div ref={joystickRef} className={`absolute bottom-4 left-4 z-20 w-20 h-20 rounded-full flex items-center justify-center ${isLightTheme ? 'bg-black/15' : 'bg-white/30 border border-white/80'}`} style={{ touchAction: 'none' }} onTouchStart={handleJoystickStart} onTouchMove={handleJoystickMove} onTouchEnd={handleJoystickEnd} onTouchCancel={handleJoystickEnd}>
          <div className="w-7 h-7 rounded-full bg-gray-400/50" style={{
