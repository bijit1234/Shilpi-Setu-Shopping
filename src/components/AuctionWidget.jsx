...
export default function AuctionWidget({ productId }) {
    const { user } = useAuth();
    ...
    const placeBid = async () => {
        if (!user)
            return alert(t('auth.signInRequired'));
        if (!auction)
            return alert(t('auction.noAuction'));
        ...
        if (numeric <= highest)
            return alert(t('auction.bidTooLow'));
        try {
    ...
    if (!auction)
        return (<div className="p-4 card rounded-lg">
    ...
    return (<div className="p-4 card rounded-lg">
      ...
      {timeLeft && (<div className="mb-3 text-sm text-red-500 font-mono">Time left: {timeLeft}</div>)}

      <div className="mb-3">
        <input type="number" value={amount === '' ? '' : String(amount)} onChange={(e) => {
            const v = e.target.value;
            setAmount(v ? Number(v) : '');
        }} placeholder={t('auction.enterBid')} className="w-full border border-[var(--border)] p-2 rounded bg-[var(--bg-2)] text-[var(--text)]" disabled={auctionEnded}/>
      </div>
      ...
      <div className="mt-4">
        ...
        <ul className="mt-2">
      {bids.map(b => (<li key={b.id} className="flex justify-between py-1 border-b border-[var(--border)] text-[var(--muted)]">
