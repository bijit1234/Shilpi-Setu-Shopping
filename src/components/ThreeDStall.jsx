...
import { initStallScene } from '../utils/stallScene';
export default function ThreeDStall({ sellerId, sellerName, products, onAddToCart, onViewDetails }) {
    const canvasRef = useRef(null);
    const sceneRef = useRef(null);
    const [selected, setSelected] = useState(null);
    const [modalOpen, setModalOpen] = useState(false);
    const billboardSpecs = useMemo(() => {
        const positions = [
    ...
    const handleAddToCart = useCallback((productId) => onAddToCart(productId), [onAddToCart]);
