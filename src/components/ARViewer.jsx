...
import { WebGLRenderer, Scene, PerspectiveCamera, Mesh, BoxGeometry, MeshStandardMaterial, HemisphereLight, TextureLoader, Vector3 } from 'three';
// ARViewer: Markerless AR using WebXR + Three.js
// Renders a sample cube in AR. Extend to load product models as needed.
export default function ARViewer({ open, onClose, imageUrl, productType }) {
    const { t } = useTranslation();
    const arRef = useRef(null);
    const rendererRef = useRef(null);
    const sessionRef = useRef(null);
    const [verticalPlaced, setVerticalPlaced] = React.useState(false);
    const verticalWorldPos = useRef(null);
    ...
    const processImageWithRemoveBg = async (imageUrl) => {
        try {
    ...
    useEffect(() => {
        if (!open || !arRef.current || isProcessingImage || !processedImageUrl || !arReady)
            return;
        let renderer = null;
        let scene = null;
        let camera = null;
        ...
        async function startAR() {
            ...
            const setPlaneOrientation = (planeObj) => {
            ...
            if (processedImageUrl) {
                ...
                loader.load(processedImageUrl, (texture) => {
                    plane = new Mesh(new BoxGeometry(0.3, 0.3, 0.01), new MeshStandardMaterial({ map: texture, transparent: true }));
                    plane.position.set(0, 0, -0.5);
                    setPlaneOrientation(plane);
                    scene.add(plane);
                }, undefined, () => {
                    // fallback to colored plane if image fails
                    plane = new Mesh(new BoxGeometry(0.3, 0.3, 0.01), new MeshStandardMaterial({ color: 0xff6600, transparent: true }));
                    plane.position.set(0, 0, -0.5);
                    setPlaneOrientation(plane);
                    scene.add(plane);
                });
    ...
    if (open && showArStartupModal && !arReady) {
        ...
        <div style={{ background: '#222', padding: 32, borderRadius: 18, color: '#fff', minWidth: 320, textAlign: 'center', boxShadow: '0 4px 32px #0008' }}>
          {isProcessingImage || !processedImageUrl ? (<>
              ...
              <button onClick={() => setARReady(true)} style={{ margin: '20px auto 0', background: '#218cf7', color: '#fff', border: 'none', borderRadius: 8, fontSize: 18, padding: '12px 33px', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 12px #0002' }}>
