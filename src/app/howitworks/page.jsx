        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-[var(--bg-2)]/90 backdrop-blur-sm rounded-3xl shadow-soft border border-[var(--heritage-gold)]/20 p-8 hover:shadow-glow transition-shadow duration-300">
            ...
            <ol className="space-y-4">
              {t('howItWorksPage.artisanSection.steps', { returnObjects: true }).map((step, i) => (<li key={i} className="flex items-start text-[var(--muted)]">
          ...
          <div className="bg-[var(--bg-2)]/90 backdrop-blur-sm rounded-3xl shadow-soft border border-[var(--heritage-gold)]/20 p-8 hover:shadow-glow transition-shadow duration-300">
            ...
            <ol className="space-y-4">
              {t('howItWorksPage.buyerSection.steps', { returnObjects: true }).map((step, i) => (<li key={i} className="flex items-start text-[var(--muted)]">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-[var(--bg-2)]/80 rounded-3xl border border-[var(--border)] p-8">
            ...
            <ul className="space-y-2 text-[var(--muted)]">
              {t('howItWorksPage.aiSection.features', { returnObjects: true }).map((feature, i) => (<li key={i} className="flex items-center"><span className="text-[var(--heritage-gold)] mr-2">✦</span> {feature}</li>))}
            </ul>
          </div>
          ...
          <div className="bg-[var(--bg-2)]/80 rounded-3xl border border-[var(--border)] p-8">
            ...
            <ul className="space-y-2 text-[var(--muted)]">
              {t('howItWorksPage.safetySection.features', { returnObjects: true }).map((feature, i) => (<li key={i} className="flex items-center"><span className="text-[var(--success)] mr-2">🛡️</span> {feature}</li>))}
            </ul>
          </div>
        </div>
