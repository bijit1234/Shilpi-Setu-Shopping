...
import { ArrowRight, Palette, ShoppingBag, Users, Award, Heart, Sparkles } from 'lucide-react';
...
function FaqCustomAccordion() {
    ...
    const faqData = [
        {
            question: t('home.faq.q1', "What is Shilpi Setu?"),
            answer: t('home.faq.a1', "Shilpi Setu is an AI-powered marketplace connecting talented artisans with buyers worldwide...")
        },
        {
            question: t('home.faq.q2', "How do I buy products on Shilpi Setu?"),
            answer: t('home.faq.a2', "Simply browse our marketplace, use voice or text search...")
        },
        {
            question: t('home.faq.q3', "How can artisans sell on Shilpi Setu?"),
            answer: t('home.faq.a3', "Sign up as a seller, create your personalized virtual stall...")
        },
        {
            question: t('home.faq.q4', "What makes Shilpi Setu different from other marketplaces?"),
            answer: t('home.faq.a4', "Shilpi Setu uniquely combines tradition with technology...")
        },
        {
            question: t('home.faq.q5', "Is Shilpi Setu free to use?"),
    ...
    return (<div className="w-full max-w-4xl mx-auto">
      <div className="space-y-4">
        {faqData.map(({ question, answer }, index) => {
            ...
            return (<div key={value} className={`rounded-xl px-4 transition-colors duration-200 border ${isOpen
              ...
              <button onClick={() => toggleItem(value)} className={`flex w-full items-center justify-between py-4 text-left text-lg font-serif font-semibold tracking-tight transition-all ${isOpen ? 'text-[var(--heritage-red)]' : 'text-[var(--heritage-brown)]'}`} aria-expanded={isOpen}>
                {question}
                <PlusIcon className={`h-5 w-5 shrink-0 text-[var(--heritage-gold)] transition-transform duration-200 ${isOpen ? 'rotate-45' : ''}`}/>
              </button>
...
export default function Home() {
    ...
    return (<div className="min-h-screen">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden heritage-bg">
        ...
        <video key={videoSources[currentVideo]} src={videoSources[currentVideo]} autoPlay muted playsInline onEnded={handleEnded} ref={(el) => {
            if (el) {
        ...
        <div className="container mx-auto relative z-30 flex items-center justify-center min-h-screen">
          <div className="flex flex-col items-center text-center space-y-8 max-w-3xl w-full mx-auto px-4 py-8 md:py-16">
            ...
            <div className="flex justify-center gap-2 mt-10 animate-slide-in-up animate-delay-500">
              {videoSources.map((src, idx) => (<button key={idx} aria-label={`Video ${idx + 1}`} className={`h-2 w-2 rounded-full transition-all duration-300 ${currentVideo === idx ? "bg-white/80" : "bg-white/40 hover:bg-white/60"}`} onClick={() => setCurrentVideo(idx)} style={{ outline: currentVideo === idx ? "2px solid #fff" : "none" }}/>))}
            </div>
          </div>
        </div>
      </section>
      ...
      {false && (<section className="py-16 md:py-24 bg-gradient-to-br from-[var(--bg-2)]/80 to-white/60 dark:to-[var(--bg-1)]/60 backdrop-blur-sm relative overflow-visible video-section">
        {/* Background Pattern */}
          ...
          <div className="relative max-w-6xl mx-auto animate-slide-in-up animate-delay-200 mb-12 md:mb-16">
            ...
            <div className="absolute -inset-2 md:-inset-4 bg-gradient-to-r from-[var(--heritage-gold)] via-[var(--heritage-red)] to-[var(--heritage-gold)] rounded-2xl md:rounded-3xl p-1">
              <div className="bg-white rounded-xl md:rounded-3xl p-1 md:p-2">
                <VideoPlayer src="https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4" poster="/api/placeholder?width=800&height=450&text=Shilpi+Setu+Craftsmanship" title={t('home.videoPlayerTitle')} description={t('home.videoPlayerDesc')} className="aspect-video rounded-lg md:rounded-2xl overflow-hidden shadow-2xl"/>
              </div>
            </div>
          </div>
        ...
        <div className="container-custom relative">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="card-glass p-8 text-center group animate-slide-in-up animate-delay-100 hover-lift">
              <div className="relative mb-8">
                <div className="w-full h-64 rounded-2xl overflow-hidden shadow-medium relative">
                  <Image src="/saree.jpg" alt={t('home.featuredCard1Title')} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"/>
            ...
            <div className="card-glass p-8 text-center group animate-slide-in-up animate-delay-200 hover-lift">
              <div className="relative mb-8">
                <div className="w-full h-64 rounded-2xl overflow-hidden shadow-medium relative">
                  <Image src="/gold&jwellery.jpg" alt={t('home.featuredCard2Title')} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"/>
            ...
            <div className="card-glass p-8 text-center group animate-slide-in-up animate-delay-300 hover-lift">
              <div className="relative mb-8">
                <div className="w-full h-64 rounded-2xl overflow-hidden shadow-medium relative">
                  <Image src="/pottery.jpg" alt={t('home.featuredCard3Title')} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"/>
            ...
            <div className="md:col-span-5 animate-slide-in-up animate-delay-300 order-1 md:order-2 flex justify-center md:justify-end">
              <div className="relative w-full max-w-sm aspect-square flex items-center justify-center rounded-3xl shadow-2xl bg-[#fdfbf7] dark:bg-[var(--card)] border-8 border-white dark:border-[var(--border)]">
                <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
                  <iframe width="100%" height="100%" src="https://www.youtube.com/embed/Ge7mo6KzMTo?autoplay=1&mute=1&loop=1&playlist=Ge7mo6KzMTo&controls=0&modestbranding=1&rel=0&showinfo=0&fs=0&disablekb=1" title="Shilpi Setu 3D Bazaar Preview" allow="autoplay; encrypted-media" allowFullScreen={false} className="aspect-square rounded-2xl overflow-hidden" style={{ minHeight: '100%', minWidth: '100%', border: 'none', pointerEvents: 'none' }} frameBorder="0"></iframe>
                </div>
      ...
      <button type="button" className="fixed bottom-6 right-6 z-50 w-14 h-14 md:w-16 md:h-16 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-full shadow-2xl hover:shadow-green-500/50 transition-all duration-300 flex items-center justify-center group hover:scale-110" title={t('donate.title', 'Donate Items for NGO')} tabIndex={0} onClick={() => setDonateModalOpen(true)} aria-label={t('donate.title', 'Donate Items for NGO')}>
