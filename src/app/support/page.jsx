      ...
      <div className="container-custom max-w-4xl mx-auto py-12 space-y-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-[var(--bg-2)]/90 backdrop-blur-sm p-6 rounded-2xl border border-[var(--heritage-gold)]/20 shadow-soft">
            ...
            <ul className="space-y-2 text-[var(--muted)]">
              <li><strong className="text-[var(--text)]">{t('support.emailLabel')}:</strong> <a href="mailto:talkto.shilpisetu@gmail.com" className="hover:text-[var(--heritage-red)] text-[var(--heritage-gold)] transition-colors">talkto.shilpisetu@gmail.com</a></li>
        ...
        <div className="bg-[var(--bg-2)]/90 backdrop-blur-sm rounded-3xl shadow-soft border border-[var(--heritage-gold)]/20 p-8 md:p-10">
          ...
          <div className="space-y-4">
            {faqs.map((faq, idx) => (<div key={idx} className="border-b border-[var(--border)] last:border-0 pb-4 last:pb-0">
                <button className="flex items-center justify-between w-full text-left py-2 focus:outline-none group" onClick={() => setOpenIndex(openIndex === idx ? null : idx)}>
                ...
                <div className={`grid transition-all duration-300 ease-in-out ${openIndex === idx ? 'grid-rows-[1fr] opacity-100 mt-2' : 'grid-rows-[0fr] opacity-0'}`}>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-[var(--muted)] list-disc pl-5 marker:text-[var(--heritage-gold)]">
              {t('support.sellerHelpList', { returnObjects: true }).map((item, i) => (<li key={i}>{item}</li>))}
            </ul>
          </div>
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-[var(--muted)] list-disc pl-5 marker:text-[var(--heritage-green)]">
              {t('support.buyerHelpList', { returnObjects: true }).map((item, i) => (<li key={i}>{item}</li>))}
            </ul>
          </div>
        </div>
      </div>
