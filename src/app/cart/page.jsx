...
import { ShoppingCart, ArrowLeft, Shield, Heart, Plus } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import Link from 'next/link';
import { useTranslation } from 'react-i18next';
import { getAnonymousCart, updateAnonymousCartQuantity, removeFromAnonymousCart, addToAnonymousCart } from '@/utils/cart';
export default function CartPage() {
    ...
    const fetchCartItems = async () => {
        if (user) {
          ...
          `)
            ...
            const itemsWithProducts = await Promise.all(anonymousCart.map(async (item) => {
                try {
    ...
    return (<div className="min-h-screen heritage-bg py-10 relative overflow-hidden">
      {/* Background Elements */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-12 border-b border-[var(--border)] pb-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              <Link href="/marketplace" className="inline-flex items-center text-[var(--muted)] hover:text-[var(--text)] text-xs font-medium mb-6 transition-colors group uppercase tracking-[0.2em]">
        ...
        {/* Feedback Toast */}
        {feedback && (<div className="fixed top-24 left-1/2 transform -translate-x-1/2 z-50 bg-[var(--bg-2)] border-l-4 border-[var(--heritage-gold)] text-[var(--text)] px-6 py-4 shadow-xl animate-fade-in flex items-center gap-4">
        ...
        {cartItems.length === 0 ? (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.1 }} className="text-center py-24 bg-[var(--bg-2)] border border-[var(--border)] flex flex-col items-center justify-center max-w-2xl mx-auto">
            <div className="mb-6">
            ...
            <Link href="/marketplace" className="inline-flex items-center px-8 py-4 bg-[var(--text)] text-[var(--bg-1)] font-bold uppercase tracking-widest hover:bg-[var(--heritage-gold)] transition-colors duration-300">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="lg:col-span-2">
              <div className="bg-[var(--bg-2)] border border-[var(--border)]">
                ...
                const itemId = 'id' in item ? item.id : item.product_id;
                return (<motion.div key={itemId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: index * 0.1 }} className="group p-6 hover:bg-[var(--bg-1)]/50 transition-colors">
                        <div className="flex flex-col sm:grid sm:grid-cols-12 gap-6 items-center">
                          ...
                          <div className="w-full sm:col-span-6 flex gap-6 items-center">
                            <div className="w-20 h-20 sm:w-24 sm:h-24 bg-[var(--bg-3)] flex-shrink-0 border border-[var(--border)] relative overflow-hidden">
                              {item.product.image_url ? (<img src={item.product.image_url} alt={item.product.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"/>) : (<div className="w-full h-full flex items-center justify-center text-[var(--muted)] text-xs">{t('cart.page.noImg', 'NO IMG')}</div>)}
                            </div>
                            <div className="min-w-0">
                              ...
                              <button onClick={() => removeFromCart(item)} disabled={!!actionLoading} className="text-xs text-[var(--muted)] hover:text-red-500 transition-colors flex items-center gap-1 group/delete">
                          ...
                          <div className="w-full sm:col-span-2 flex justify-center">
                            <div className="flex items-center border border-[var(--border)] bg-[var(--bg-1)] h-8">
                              <button onClick={() => updateQuantity(item, item.quantity - 1)} disabled={actionLoading === itemId} className="w-8 h-full flex items-center justify-center hover:bg-[var(--bg-3)] transition-colors text-[var(--muted)] hover:text-[var(--text)] disabled:opacity-30">
                                -
                              </button>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="lg:col-span-1">
              <div className="bg-[var(--bg-2)] border border-[var(--border)] p-8 sticky top-24">
            ...
            }} disabled={cartItems.length === 0 || checkoutLoading} className={`
                ...
                : 'bg-[var(--text)] text-[var(--bg-1)] hover:bg-[var(--heritage-gold)] hover:text-white border border-[var(--text)] hover:border-[var(--heritage-gold)]'}
                  `}>
                  {checkoutLoading ? (<span className="flex items-center justify-center gap-2">
        ...
        {/* Wishlist Section */}
        {wishlistItems.length > 0 && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mt-16 pt-16 border-t border-[var(--border)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {wishlistItems.map((item) => {
                ...
                return (<div key={item.id} className="group bg-[var(--bg-2)] border border-[var(--border)] p-4 flex flex-col">
                    <Link href={`/product/${item.product_id}`} className="block relative aspect-square bg-[var(--bg-3)] mb-4 overflow-hidden">
                      {item.image_url ? (<img src={item.image_url} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/>) : (<div className="w-full h-full flex items-center justify-center text-[var(--muted)] text-xs">{t('cart.page.noImg', 'NO IMG')}</div>)}
                    </Link>
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      ...
                      <button onClick={() => addToCartFromWishlist(item)} disabled={!!actionLoading} className="p-2 border border-[var(--border)] hover:bg-[var(--text)] hover:text-[var(--bg-1)] transition-colors disabled:opacity-50" title={isInCart ? 'Add another' : 'Add to Cart'}>
