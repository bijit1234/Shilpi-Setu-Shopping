...
export default function SellerAuctionsList({ sellerId, refreshTrigger }) {
    ...
    const fetchAuctions = useCallback(async () => {
        ...
        try {
            ...
            const withTitles = rows.map((r) => ({ ...r, product_title: productMap[r.product_id] || r.product_id }));
    ...
    return (<div>
      <div className="grid gap-4 sm:gap-6">
        {auctions.map(a => (<div key={a.id} className="relative overflow-hidden rounded-xl bg-[var(--bg-2)] dark:bg-[var(--bg-2)] border-2 border-amber-300 dark:border-amber-700/50 shadow-md hover:shadow-xl transition-all duration-300 group">
            {/* Status Badge */}
            <div className="absolute top-4 right-4 z-10">
              <span className={`px-3 py-1 text-xs font-bold rounded-full shadow-lg ${a.status === 'running'
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-4 sm:gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    {a.starts_at && (<div className="flex items-center gap-2 text-sm">
                        <span className="text-lg">🕐</span>
                        <div>
                    ...
                    {a.ends_at && (<div className="flex items-center gap-2 text-sm sm:col-span-2">
                        <span className="text-lg">⏰</span>
                        <div>
                ...
                <div className="flex flex-col xs:flex-row lg:flex-col gap-2 lg:w-48">
                  {a.status !== 'completed' && (<button onClick={() => endAuction(a.id)} className="flex-1 lg:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-md hover:shadow-lg transform hover:scale-105 transition-all">
                      <span>🏁</span>
                      <span>{t('auction.endAuctionButton', 'End Auction')}</span>
                    </button>)}
                  <button onClick={() => deleteAuction(a.id)} className="flex-1 lg:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white rounded-lg bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 shadow-md hover:shadow-lg transform hover:scale-105 transition-all">
                    <span>🗑️</span>
