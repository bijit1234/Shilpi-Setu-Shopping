...
export default function ProfileManager({ profile, products, onProfileUpdate }) {
    ...
    const generateStoreDescription = async () => {
        try {
            ...
            const description = await ai.generateStoreDescription(formData.name, formData.bio, products.map(p => ({ title: p.title, category: p.category })));
    ...
    if (!profile) {
        return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="card rounded-xl border p-6 mb-6">
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="card rounded-xl border p-6 mb-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setIsEditing(!isEditing)} className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-4">
            <div className="w-24 h-24 rounded-full flex items-center justify-center overflow-hidden bg-[var(--bg-2)]">
              {imagePreview ? (<Image src={imagePreview} alt="Profile" width={96} height={96} className="w-full h-full object-cover"/>) : (<User className="w-12 h-12 text-orange-600"/>)}
            </div>
            
            {isEditing && (<div>
                <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden"/>
                <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-[var(--bg-2)] text-[var(--text)] rounded-lg hover:bg-[var(--bg-3)] transition-colors">
          ...
          {isEditing ? (<div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <input type="text" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} className="w-full px-3 py-2 pr-10 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400 bg-[var(--bg-2)] border-[var(--border)] text-[var(--text)]" placeholder={t('seller.profile.fields.storeName.placeholder')}/>
                  <button type="button" onClick={listeningField === 'name' ? handleStopListening : () => handleStartListening('name')} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full bg-white hover:bg-gray-100" title={listeningField === 'name' ? 'Listening...' : 'Speak'}>
              ...
              <div>
                ...
                <div className="relative">
                  <textarea value={formData.bio} onChange={(e) => setFormData(prev => ({ ...prev, bio: e.target.value }))} rows={3} className="w-full px-3 py-2 pr-10 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400 bg-[var(--bg-2)] border-[var(--border)] text-[var(--text)]" placeholder={t('seller.profile.fields.bio.placeholder')}/>
                  <button type="button" onClick={listeningField === 'bio' ? handleStopListening : () => handleStartListening('bio')} className="absolute right-2 top-2 p-1 rounded-full bg-white hover:bg-gray-100" title={listeningField === 'bio' ? 'Listening...' : 'Speak'}>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button onClick={generateStoreDescription} disabled={aiLoading} className="flex items-center text-sm text-orange-600 hover:text-orange-700 disabled:opacity-50">
                    <Sparkles className="w-4 h-4 mr-1"/>
                    {aiLoading ? t('seller.profile.generating') : t('seller.profile.aiGenerate')}
                  </button>
                </div>
                <div className="relative">
                  <textarea value={formData.store_description} onChange={(e) => setFormData(prev => ({ ...prev, store_description: e.target.value }))} rows={4} className="w-full px-3 py-2 pr-10 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400 bg-[var(--bg-2)] border-[var(--border)] text-[var(--text)]" placeholder={t('seller.profile.fields.description.placeholder')}/>
                  <button type="button" onClick={listeningField === 'store_description' ? handleStopListening : () => handleStartListening('store_description')} className="absolute right-2 top-2 p-1 rounded-full bg-white hover:bg-gray-100" title={listeningField === 'store_description' ? 'Listening...' : 'Speak'}>
              ...
              <div>
                ...
                <input type="text" value={formData.upi_id} onChange={(e) => setFormData(prev => ({ ...prev, upi_id: e.target.value }))} className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400 bg-[var(--bg-2)] border-[var(--border)] text-[var(--text)]" placeholder="e.g. yourname@upi"/>
              </div>

              <button onClick={handleSave} disabled={loading} className="w-full px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors disabled:opacity-50 flex items-center justify-center">
      ...
      {!isEditing && (<div className="mt-6 p-4 rounded-lg border border-[var(--border)] bg-[var(--bg-2)]">
