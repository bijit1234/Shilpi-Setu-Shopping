...
export default function SellerAnalytics({ sellerId }) {
    ...
    const [rawGuidance, setRawGuidance] = useState('');
        ...
        const load = async () => {
            ...
            try {
                ...
                try {
                    ...
                    fetchedRawGuidance = tips;
                    try {
                        const tr = await translateText(tips, currentLanguage);
                        fetchedGuidance = tr;
                    }
                    catch {
                        fetchedGuidance = tips;
                    }
                }
                catch {
                    if (top.length > 0) {
                        fetchedGuidance = t('seller.analyticsGuidance.audienceEngagingMost', { titles: top.map(tp => tp.title).join(', ') });
                    }
                    else if (fetchedTotalViews === 0) {
                        fetchedGuidance = t('seller.analyticsGuidance.noRecentVisits');
                    }
                    else {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl shadow-lg border border-orange-100 dark:border-[var(--border)] bg-gradient-to-br from-orange-100/80 via-yellow-50/80 to-white/70 dark:from-orange-900/30 dark:via-yellow-900/30 dark:to-black/40 backdrop-blur-md bg-opacity-70 p-6 flex flex-col">
          ...
          {topProducts.length === 0 ? (<div className="text-sm text-[var(--muted)]">{t('seller.analyticsShort.noProductViewsYet')}</div>) : (<ul className="space-y-1">
              {topProducts.map(p => (<li key={p.id} className="flex justify-between text-sm text-[var(--muted)]">
      ...
      {collabStats.activeCollaborations > 0 && (<div className="rounded-2xl shadow-xl border border-yellow-200 dark:border-yellow-900 bg-gradient-to-br from-yellow-100/80 via-orange-50/80 to-white/70 dark:from-yellow-900/30 dark:via-orange-900/30 dark:to-black/40 backdrop-blur-md bg-opacity-70 p-6">
              ...
              <div className="w-full flex flex-col items-center">
                {collabStats.collaborativeProducts > 0 ? (<>
                    <div className="w-full h-3 rounded-full bg-yellow-200 dark:bg-yellow-900/30 overflow-hidden mt-2">
                      <div className="h-3 rounded-full bg-gradient-to-r from-orange-400 to-yellow-500 dark:from-orange-600 dark:to-yellow-400 transition-all duration-700" style={{ width: `${Math.round((collabStats.collaborativeProducts / (topProducts.length + collabStats.collaborativeProducts)) * 100)}%` }}/>
                    </div>
      ...
      <div className="rounded-2xl shadow-lg border border-orange-200 dark:border-[var(--border)] bg-gradient-to-br from-orange-50/80 via-yellow-100/80 to-white/70 dark:from-orange-900/30 dark:via-yellow-900/30 dark:to-black/40 backdrop-blur-md bg-opacity-70 p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {toBullets(guidance).length > 1 ? (<ul className="list-disc list-inside space-y-1 text-sm text-[var(--text)]">
                {toBullets(guidance).map((item, idx) => (<li key={idx}>{item}</li>))}
              </ul>) : (<div className="text-sm text-[var(--text)] leading-relaxed">{guidance}</div>)}
          </div>
        </div>
        <div className="mt-6 pt-4 border-t border-orange-100 dark:border-[var(--border)]">
          ...
          <div className="flex flex-col xs:flex-row gap-2">
            <input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder={t('seller.analyticsShort.askPlaceholder')} className="flex-1 px-3 py-2 text-base rounded-lg focus:outline-none bg-[var(--bg-2)] border-orange-100 dark:border-[var(--border)] text-[var(--text)]"/>
            <button disabled={qaLoading || !question.trim()} onClick={async () => {
            try {
          ...
          {answer && (toBullets(answer).length > 1 ? (<ul className="mt-4 list-disc list-inside space-y-1 text-sm text-[var(--text)]">
                {toBullets(answer).map((item, idx) => (<li key={idx}>{item}</li>))}
              </ul>) : (<div className="mt-4 text-sm text-[var(--text)] whitespace-pre-wrap">{answer}</div>))}
        </div>
      </div>
    </div>);
}
