...
export async function GET(req) {
    try {
        ...
        const threadIdsA = userThreadsA.map((t) => t.thread_id);
        const threadIdsB = userThreadsB.map((t) => t.thread_id);
        const commonThreadIds = threadIdsA.filter((id) => threadIdsB.includes(id));
...
export async function POST(req) {
    try {
        ...
        if (threadError)
            return NextResponse.json({ error: threadError.message }, { status: 500 });
        // Add participants
        const participantsToInsert = participantIds.map((uid) => ({ thread_id: thread.id, user_id: uid }));
