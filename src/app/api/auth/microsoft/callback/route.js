...
const supabaseAdmin = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, // Use service role key to bypass RLS
{
    auth: {
    ...
    try {
        ...
        if (existingProfile) {
            ...
            try {
                ...
                if (!linkError && linkData?.properties?.action_link) {
                    console.log('Generated native sign in link for existing Microsoft user');
        ...
        if (authUserId) {
            ...
            try {
                ...
                if (!linkError && linkData?.properties?.action_link) {
                    console.log('Generated native sign in link for new Microsoft user');
                    return NextResponse.redirect(linkData.properties.action_link);
                }
                else if (linkError) {
