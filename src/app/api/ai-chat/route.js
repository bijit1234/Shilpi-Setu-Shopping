...
import { generateRAGResponse, generateTemplateResponse } from '@/lib/llm-generator';
    ...
    try {
        ...
        if (parsed.keywords.length > 0) {
            ...
            if (!textError && textData) {
                ...
                // Add text results
                textData.forEach((p) => uniqueMap.set(p.id, p));
        ...
        const context = {
            ...
            userPreferences: userPreferences ? {
                ...
                recentSearches: userPreferences.common_search_terms?.map((t) => t.term),
            } : undefined,
            isFollowUp,
        ...
        if (userId) {
            try {
                ...
                await supabase.from('conversation_history').insert({
                    ...
                    query_context: {
                        productIds: topResults.slice(0, 3).map((p) => p.id),
                        productCount: topResults.length,
                    },
                });
