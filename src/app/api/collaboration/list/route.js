...
export async function GET(request) {
    try {
        ...
        if (userProfile.role !== 'seller') {
            return NextResponse.json({ error: 'Only sellers can have collaborations' }, { status: 403 });
        }
      ...
      `)
        ...
        if (fetchError) {
            ...
            return NextResponse.json({ error: 'Failed to fetch collaborations', details: fetchError.message }, { status: 500 });
        }
        // Enhance collaborations with additional info
        const enhancedCollaborations = await Promise.all((collaborations || []).map(async (collab) => {
