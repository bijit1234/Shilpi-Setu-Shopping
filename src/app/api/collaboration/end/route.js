...
export async function POST(request) {
    try {
        ...
        if (!collaborationId || !userId) {
            return NextResponse.json({ error: 'Collaboration ID and User ID are required' }, { status: 400 });
        }
        ...
        if (collaboration.status !== 'accepted') {
            return NextResponse.json({ error: 'Only active collaborations can be ended' }, { status: 400 });
        }
        ...
        if (deleteError) {
            ...
            return NextResponse.json({ error: 'Failed to end collaboration', details: deleteError.message }, { status: 500 });
        }
        // Log the activity
        await supabase
...
export async function DELETE(request) {
    try {
        ...
        if (collaboration.initiator_id !== userId) {
            return NextResponse.json({ error: 'Only the initiator can cancel a pending collaboration request' }, { status: 403 });
        }
        if (collaboration.status !== 'pending') {
            return NextResponse.json({ error: 'Only pending collaboration requests can be cancelled' }, { status: 400 });
        }
        ...
        if (deleteError) {
            ...
            return NextResponse.json({ error: 'Failed to cancel collaboration', details: deleteError.message }, { status: 500 });
        }
        return NextResponse.json({
            success: true,
