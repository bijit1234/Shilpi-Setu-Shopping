...
export async function GET(request) {
    try {
        ...
        if (sellersError) {
            ...
            return NextResponse.json({ error: 'Failed to search sellers', details: sellersError.message }, { status: 500 });
        }
        ...
        const enhancedSellers = await Promise.all((sellers || []).map(async (seller) => {
            ...
            const existingCollab = existingCollabs?.find(c => (c.initiator_id === seller.id || c.partner_id === seller.id));
        ...
        const availableSellers = enhancedSellers.filter(s => !s.collaborationStatus ||
