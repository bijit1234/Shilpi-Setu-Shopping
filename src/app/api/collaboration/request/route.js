...
export async function POST(request) {
    try {
        ...
        const { initiatorId, partnerId, message, collaborationName, collaborationDescription, terms } = body;
        // Validation
        if (!initiatorId || !partnerId) {
            return NextResponse.json({ error: 'Initiator ID and Partner ID are required' }, { status: 400 });
        }
        if (initiatorId === partnerId) {
        ...
        if (initiator.role !== 'seller' || partner.role !== 'seller') {
            return NextResponse.json({ error: 'Both users must be sellers to collaborate' }, { status: 403 });
        }
        ...
        if (collabError) {
            ...
            return NextResponse.json({ error: 'Failed to create collaboration request', details: collabError.message }, { status: 500 });
        }
