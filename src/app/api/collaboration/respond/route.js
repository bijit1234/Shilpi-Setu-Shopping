...
export async function POST(request) {
    try {
        ...
        const { collaborationId, userId, action, // 'accept' or 'reject'
        responseMessage } = body;
        // Validation
        if (!collaborationId || !userId || !action) {
            return NextResponse.json({ error: 'Collaboration ID, User ID, and action are required' }, { status: 400 });
        }
        ...
        if (collaboration.partner_id !== userId) {
            return NextResponse.json({ error: 'Only the collaboration partner can respond to this request' }, { status: 403 });
        }
        // Check if already responded
        if (collaboration.status !== 'pending') {
            return NextResponse.json({ error: `This collaboration request has already been ${collaboration.status}` }, { status: 400 });
        }
        ...
        if (action === 'accept') {
            ...
            if (updateError) {
                ...
                return NextResponse.json({ error: 'Failed to update collaboration', details: updateError.message }, { status: 500 });
            }
            ...
            if (deleteError) {
                ...
                return NextResponse.json({ error: 'Failed to delete collaboration', details: deleteError.message }, { status: 500 });
            }
