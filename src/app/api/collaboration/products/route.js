...
export async function POST(request) {
    try {
        ...
        const { collaborationId, productId, primarySellerId, contributionDetails, revenueSplitOverride } = body;
        // Validation
        if (!collaborationId || !productId || !primarySellerId) {
            return NextResponse.json({ error: 'Collaboration ID, Product ID, and Primary Seller ID are required' }, { status: 400 });
        }
        ...
        if (collaboration.status !== 'accepted') {
            return NextResponse.json({ error: 'Can only add products to active collaborations' }, { status: 400 });
        }
        ...
        if (collaboration.initiator_id !== primarySellerId && collaboration.partner_id !== primarySellerId) {
            return NextResponse.json({ error: 'Primary seller must be part of the collaboration' }, { status: 403 });
        }
        ...
        if (product.seller_id !== collaboration.initiator_id &&
            ...
            return NextResponse.json({ error: 'Product must belong to one of the collaborators' }, { status: 403 });
        }
        ...
        if (existingCollabProduct) {
            return NextResponse.json({ error: 'Product is already part of another collaboration' }, { status: 409 });
        }
        ...
        if (insertError) {
            ...
            return NextResponse.json({ error: 'Failed to add product to collaboration', details: insertError.message }, { status: 500 });
        }
        // Log the activity
        await supabase
    ...
    try {
      ...
      `)
        ...
        if (error) {
            ...
            return NextResponse.json({ error: 'Failed to fetch collaborative products', details: error.message }, { status: 500 });
        }
        return NextResponse.json({
            success: true,
...
export async function DELETE(request) {
    try {
        ...
        if (fetchError || !collabProduct) {
            return NextResponse.json({ error: 'Collaborative product not found' }, { status: 404 });
        }
        ...
        const rawCollab = collabProduct.collaboration;
        ...
        if (deleteError) {
            ...
            return NextResponse.json({ error: 'Failed to remove product from collaboration', details: deleteError.message }, { status: 500 });
        }
        // Log the activity
        await supabase
            .from('collaboration_activity')
