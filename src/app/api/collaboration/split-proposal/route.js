...
export async function GET(request) {
    try {
        ...
        return NextResponse.json({ error: msg || 'Failed to fetch split proposals' }, { status: 500 });
    }
}
...
export async function POST(request) {
    try {
        ...
        const { collaborativeProductId, collaborationId, proposedSplit, // { initiator: 60, partner: 40 }
        reason, userId // Pass userId from client
         } = body;
        // Validate inputs
        ...
        if (proposedSplit.initiator + proposedSplit.partner !== 100) {
            return NextResponse.json({ error: 'Split percentages must add up to 100' }, { status: 400 });
        }
        ...
        if (collabError || !collaboration) {
            return NextResponse.json({ error: 'Collaboration not found or you are not authorized' }, { status: 403 });
        }
        ...
        const { data: existingProposal } = await supabase
            ...
            .single();
        if (existingProposal) {
            return NextResponse.json({ error: 'There is already a pending proposal for this product. Please wait for it to be resolved.' }, { status: 400 });
        }
        // Create the proposal
        ...
        return NextResponse.json({ error: msg || 'Failed to create split proposal' }, { status: 500 });
    }
}
...
export async function PATCH(request) {
    try {
      ...
      `)
        ...
        // Verify user is part of the collaboration
        const collaboration = proposal.collaboration;
        if (!collaboration || (collaboration.initiator_id !== userId && collaboration.partner_id !== userId)) {
            return NextResponse.json({ error: 'You are not authorized to respond to this proposal' }, { status: 403 });
        }
        // Verify proposal is still pending
        if (proposal.status !== 'pending') {
            return NextResponse.json({ error: `This proposal has already been ${proposal.status}` }, { status: 400 });
        }
        // Update the proposal
        ...
        return NextResponse.json({ error: msg || 'Failed to respond to proposal' }, { status: 500 });
    }
}
