...
import { User, Edit3, Camera, MapPin, Package, Heart, Settings, LogOut, ShoppingBag, Truck, CreditCard, Bell, ChevronRight, Home, Plus, Trash2, CheckCircle, Search, Filter, X, LayoutDashboard, MessageCircle, Menu } from 'lucide-react';
...
export default function ProfilePage() {
    ...
    const handleDeleteAddress = async (id) => {
        if (!confirm(t('profile.confirmDeleteAddress')))
            return;
        try {
    ...
    // --- Render Components ---
    const SidebarItem = ({ id, icon: Icon, label, badge }) => (<button onClick={() => { setActiveTab(id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${activeTab === id
      ...
      {badge && badge > 0 ? (<span className="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{badge}</span>) : null}
    ...
    const StatusBadge = ({ status }) => {
        ...
        return (<span className={`px-2 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${styles[status]}`}>
        {status}
      </span>);
    };
    if (loading || !profile)
        return (<div className="min-h-screen flex items-center justify-center bg-[var(--bg-1)]">
      <div className="animate-spin w-12 h-12 border-4 border-[var(--heritage-gold)] border-t-transparent rounded-full"></div>
    </div>);
    return (<div className="min-h-screen bg-[var(--bg-1)] font-sans text-[var(--text)]">
      {/* Background Decor */}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          `}>
            <div className="card-glass rounded-2xl p-6 lg:sticky lg:top-24 border border-[var(--border)] h-full lg:h-auto overflow-y-auto">
              ...
              <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden absolute top-4 right-4 p-2 hover:bg-[var(--bg-2)] rounded-lg transition-colors">
                <X className="w-5 h-5"/>
              </button>
              ...
              <div className="flex flex-col items-center mb-8">
                <div className="relative mb-4 group">
                  <div className="w-24 h-24 rounded-full p-1 bg-gradient-to-br from-[var(--heritage-gold)] to-[var(--heritage-red)]">
                    <div className="w-full h-full rounded-full overflow-hidden bg-[var(--bg-1)] relative">
                      {form.profile_image ? (<img src={form.profile_image} alt={form.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center bg-[var(--bg-2)]">
                          <User className="w-8 h-8 text-[var(--muted)]"/>
                        </div>)}
                      {/* Hover Edit Overlay */}
                      <button onClick={handleAvatarClick} className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {profile.role === 'seller' && (<Link href="/dashboard" onClick={() => setIsSidebarOpen(false)} className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-2 bg-[var(--text)] text-[var(--bg-1)] rounded-xl font-bold text-sm hover:bg-[var(--heritage-gold)] hover:text-white transition-all shadow-md">
              ...
              <div className="mt-8 pt-6 border-t border-[var(--border)]">
                <button onClick={() => signOut()} className="w-full flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-xl transition-colors font-medium">
          ...
          <main className="flex-1 min-w-0">
            ...
            <div className="lg:hidden mb-6">
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="w-full flex items-center justify-between px-6 py-4 card-glass rounded-2xl border border-[var(--border)] hover:bg-[var(--bg-2)] transition-colors">
                <div className="flex items-center gap-3">
                  {form.profile_image ? (<img src={form.profile_image} alt={form.name} className="w-10 h-10 rounded-full object-cover"/>) : (<div className="w-10 h-10 rounded-full bg-[var(--bg-2)] flex items-center justify-center">
            ...
            {activeTab === 'notifications' && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="max-w-3xl">
            ...
            {activeTab === 'messages' && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="max-w-4xl">
            ...
            {activeTab === 'overview' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                {/* Welcome Banner */}
                ...
                <div className="card-glass rounded-2xl p-6 border border-[var(--border)]">
                  ...
                  <div className="space-y-4">
                    {orders.slice(0, 2).map(order => (<div key={order.id} className="flex items-center justify-between p-4 bg-[var(--bg-2)]/50 rounded-xl border border-[var(--border)]">
            ...
            {activeTab === 'orders' && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                ...
                <div className="space-y-4">
                  {orders.map(order => (<div key={order.id} className="card-glass rounded-2xl border border-[var(--border)] overflow-hidden">
                      ...
                      <div className="p-6">
                        {order.items.map((item, idx) => (<div key={idx} className="flex items-center gap-4 mb-4 last:mb-0">
                            <div className="w-16 h-16 bg-[var(--bg-3)] rounded-lg flex-shrink-0 flex items-center justify-center">
                              {item.image ? (<img src={item.image} className="w-full h-full object-cover rounded-lg"/>) : <Package className="w-8 h-8 text-[var(--muted)]"/>}
                            </div>
                            <div>
            ...
            {activeTab === 'wishlist' && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                {wishlistCount > 0 ? (<div className="text-center py-20 card-glass rounded-2xl border border-[var(--border)]">
                  ...
                  </div>) : (<div className="text-center py-20 card-glass rounded-2xl border border-[var(--border)]">
            ...
            {activeTab === 'addresses' && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold heritage-title">{t('profile.savedAddresses')}</h2>
                  <button onClick={() => setShowAddAddressModal(true)} className="flex items-center gap-2 px-4 py-2 bg-[var(--heritage-gold)] text-white rounded-xl font-bold text-sm shadow-lg hover:bg-[var(--heritage-red)] transition-colors">
                    <Plus className="w-4 h-4"/> {t('profile.addNewAddressBtn')}
                  </button>
                </div>

                {fetchingAddresses ? (<div className="text-center py-10"><div className="animate-spin w-8 h-8 border-4 border-[var(--heritage-gold)] border-t-transparent rounded-full mx-auto"></div></div>) : addresses.length === 0 ? (<div className="text-center py-12 card-glass rounded-2xl border border-[var(--border)]">
                    <MapPin className="w-12 h-12 text-[var(--muted)] mx-auto mb-3"/>
                    <p className="text-[var(--muted)]">{t('profile.noAddresses')}</p>
                  </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {addresses.map(addr => (<div key={addr.id} className={`card-glass p-6 rounded-2xl border ${addr.is_default ? 'border-[var(--heritage-gold)] ring-1 ring-[var(--heritage-gold)]/20' : 'border-[var(--border)]'}`}>
            ...
            {/* Add Address Modal */}
            <AnimatePresence>
              {showAddAddressModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[var(--bg-2)] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-[var(--border)]">
                    ...
                    <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                          ...
                          <input type="text" className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.name} onChange={e => setNewAddress({ ...newAddress, name: e.target.value })} placeholder="John Doe"/>
                        </div>
                        <div className="col-span-2">
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.streetAddress')}</label>
                          <input type="text" className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.street} onChange={e => setNewAddress({ ...newAddress, street: e.target.value })} placeholder="123 Main St, Apt 4B"/>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.city')}</label>
                          <input type="text" className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.city} onChange={e => setNewAddress({ ...newAddress, city: e.target.value })} placeholder="New York"/>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.state')}</label>
                          <input type="text" className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.state} onChange={e => setNewAddress({ ...newAddress, state: e.target.value })} placeholder="NY"/>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.zipCode')}</label>
                          <input type="text" className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.zip} onChange={e => setNewAddress({ ...newAddress, zip: e.target.value })} placeholder="10001"/>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.phone')}</label>
                          <input type="text" className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.phone} onChange={e => setNewAddress({ ...newAddress, phone: e.target.value })} placeholder="+1 234 567 890"/>
                        </div>
                        <div className="col-span-2 grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.addressType')}</label>
                            <select className="w-full p-2.5 bg-[var(--bg-1)] border border-[var(--border)] rounded-lg focus:ring-2 focus:ring-[var(--heritage-gold)] focus:border-transparent outline-none" value={newAddress.type} onChange={e => setNewAddress({ ...newAddress, type: e.target.value })}>
                          ...
                          <div className="flex items-center">
                            <label className="flex items-center gap-2 cursor-pointer mt-5">
                              <input type="checkbox" className="w-4 h-4 text-[var(--heritage-gold)] rounded focus:ring-[var(--heritage-gold)]" checked={newAddress.is_default} onChange={e => setNewAddress({ ...newAddress, is_default: e.target.checked })}/>
                    ...
                    <div className="p-6 border-t border-[var(--border)] flex justify-end gap-3 bg-[var(--bg-1)]/50">
                      <button onClick={() => setShowAddAddressModal(false)} className="px-4 py-2 rounded-xl font-medium hover:bg-[var(--bg-3)] transition-colors">
                        {t('common.cancel')}
                      </button>
                      <button onClick={handleAddAddress} className="px-6 py-2 bg-[var(--heritage-gold)] text-white rounded-xl font-bold hover:bg-[var(--heritage-red)] transition-colors shadow-lg">
            ...
            {activeTab === 'settings' && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="max-w-2xl">
                ...
                <div className="card-glass p-6 rounded-2xl border border-[var(--border)] mb-6">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => editMode ? handleSaveProfile() : setEditMode(true)} className={`text-sm font-bold ${editMode ? 'text-green-600' : 'text-[var(--heritage-gold)]'}`}>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-[var(--muted)] uppercase mb-1">{t('profile.fullName')}</label>
                      {editMode ? (<input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full p-2 bg-[var(--bg-2)] border border-[var(--border)] rounded-lg focus:border-[var(--heritage-gold)] outline-none"/>) : (<p className="font-medium">{form.name}</p>)}
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      {editMode ? (<textarea value={form.bio} onChange={e => setForm({ ...form, bio: e.target.value })} className="w-full p-2 bg-[var(--bg-2)] border border-[var(--border)] rounded-lg focus:border-[var(--heritage-gold)] outline-none h-24"/>) : (<p className="font-medium">{form.bio || 'No bio provided'}</p>)}
                    </div>
                  </div>
                </div>
