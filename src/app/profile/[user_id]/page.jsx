...
export default function PublicProfilePage() {
    ...
    const handleFollowClick = async () => {
        if (!user?.id || !user_id || user.id === user_id)
            return;
        setFollowersLoading(true);
        if (isFollowing) {
    ...
    if (!profile)
        return <div className="min-h-screen flex items-center justify-center text-[var(--muted)]">Profile not found.</div>;
    return (<main className="min-h-screen heritage-bg relative overflow-hidden">
        ...
        <div className="card-glass flex flex-col items-center rounded-3xl shadow-2xl p-8 hover-lift">
          ...
          <div className="relative group mb-6">
            <div className="avatar-indian-frame">
              <div className="w-28 h-28 rounded-full bg-gradient-to-br from-[var(--saffron)] via-[var(--turquoise)] to-[var(--maroon)] flex items-center justify-center border-2 border-[var(--border)] overflow-hidden">
                {profile.profile_image ? (<img src={profile.profile_image} alt="avatar" className="w-full h-full object-cover rounded-full"/>) : (<span className="text-4xl font-bold text-white">{profile.name?.[0] || '?'}</span>)}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-center items-center gap-3 mb-4">
            ...
            {user && user.id !== user_id && (<>
                <button className={`btn-indian-secondary px-4 py-1 md:py-2 rounded-xl flex items-center gap-2 text-sm md:text-base ${followersLoading ? 'opacity-60 pointer-events-none' : ''}`} onClick={handleFollowClick} disabled={followersLoading}>
                  <Users className="w-4 h-4"/>
                  {isFollowing ? 'Following' : 'Follow'}
                </button>
                <a href={`/dm?userId=${user_id}`} className="btn-indian-primary px-4 py-1 md:py-2 rounded-xl flex items-center gap-2 text-sm md:text-base" title="Message" onClick={e => {
                e.preventDefault();
                window.location.href = `/dm?userId=${user_id}`;
            }}>
