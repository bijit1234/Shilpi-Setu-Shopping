...
function SignUpContent() {
    ...
    const handleMicrosoftSignUp = async (selectedRole) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-[var(--bg-1)] via-[var(--bg-2)] to-[var(--bg-3)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      {/* Background Elements */}
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-3">
              <div className="relative w-32 h-32 flex items-center justify-center">
                <Image src="/kalamitra-symbol.png" alt="Shilpi Setu Symbol" fill className="object-contain drop-shadow-lg"/>
              </div>
              <div>
          ...
          <div className="card-glass p-8 space-y-8">
            ...
            <div className="text-center space-y-4">
              <div className="relative w-24 h-24 mx-auto">
                <Image src="/kalamitra-symbol.png" alt="Shilpi Setu Symbol" fill className="object-contain"/>
              </div>
              <div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              {error && (<div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 px-4 py-3 rounded-xl text-sm">
                  {error}
                </div>)}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input id="name" name="name" type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 bg-[var(--bg-2)] border border-[var(--border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--saffron)] focus:border-transparent transition-all duration-200 text-[var(--text)] placeholder-[var(--text-muted)]" placeholder={t('auth.enterFullName')}/>
                </div>
                ...
                <div>
                  ...
                  <input id="email" name="email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 bg-[var(--bg-2)] border border-[var(--border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--saffron)] focus:border-transparent transition-all duration-200 text-[var(--text)] placeholder-[var(--text-muted)]" placeholder={t('auth.enterYourEmail')}/>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input id="password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="new-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-[var(--bg-2)] border border-[var(--border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--saffron)] focus:border-transparent transition-all duration-200 text-[var(--text)] placeholder-[var(--text-muted)] pr-12" placeholder={t('auth.password')}/>
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button type="button" onClick={() => setRole('buyer')} className={`p-4 border-2 rounded-xl text-center transition-all duration-200 group ${role === 'buyer'
            ...
            : 'bg-[var(--bg-3)] group-hover:bg-gradient-to-br group-hover:from-[var(--saffron)] group-hover:to-[var(--turquoise)]'}`}>
                    ...
                    <button type="button" onClick={() => setRole('seller')} className={`p-4 border-2 rounded-xl text-center transition-all duration-200 group ${role === 'seller'
            ...
            setShowRoleModal(true);
                  ...
                  {microsoftLoading ? (<div className="flex items-center">
                      <div className="w-5 h-5 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin mr-2"></div>
                      {t('auth.signingIn')}
                    </div>) : (<div className="flex items-center">
                      <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
      ...
      {showRoleModal && (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--bg-2)] border border-[var(--border)] shadow-2xl rounded-2xl p-8 max-w-md w-full space-y-6">
            ...
            }} disabled={googleLoading || microsoftLoading} className="w-full flex items-center justify-center px-6 py-4 border border-[var(--border)] rounded-xl text-[var(--text)] bg-[var(--bg-2)] hover:bg-[var(--bg-3)] transition-all duration-200 disabled:opacity-50 group">
            ...
            <button onClick={() => setShowRoleModal(false)} className="w-full px-4 py-2 text-sm text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">
              {t('common.cancel')}
            </button>
          </div>
        </div>)}
