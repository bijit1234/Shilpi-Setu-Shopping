...
const ReelSkeleton = () => (<div className="rounded-xl shadow-lg bg-[var(--bg)] border border-[var(--border)] overflow-hidden flex flex-col mb-8 animate-pulse w-full">
...
const ReelsPage = () => {
    ...
    const [cursor, setCursor] = useState(null); // last reel's created_at
    ...
    // Comment UI state
    const [openCommentReelId, setOpenCommentReelId] = useState(null);
    ...
    const fetchReels = async (cursorValue, isInitial = false, retryAttempt = 0) => {
        if (loading)
            return;
    ...
    useEffect(() => {
        ...
        const fetchLikedReels = async () => {
            ...
            const { data, error } = await supabase
                ...
                .eq('user_id', user.id);
            if (!error && data) {
                setLikedReels(data.map((row) => row.reel_id));
            }
        };
        fetchLikedReels();
    }, [user?.id]);
    ...
    return (<div className="min-h-screen bg-[var(--bg-2)] flex flex-col items-center py-4">
      <div className="w-full max-w-md md:max-w-2xl flex items-center justify-between mb-6 px-3 sm:px-0">
        ...
        {user?.id && (<button className="flex items-center justify-center w-10 h-10 rounded-full bg-purple-600 hover:bg-purple-700 text-white text-2xl shadow-lg transition-colors" title="Create Reel" onClick={() => setShowUploadModal(true)}>
            <span className="pb-1">+</span>
          </button>)}
        {/* Upload Reel Modal */}
        {showUploadModal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 px-2 sm:px-0">
            <div className="rounded-xl shadow-xl w-full max-w-md relative border border-[var(--border)] p-4 sm:p-8 flex flex-col transition-colors" style={{ background: 'var(--bg-2)', color: 'var(--text)' }}>
              <button className="absolute top-2 right-2 sm:top-3 sm:right-3 text-2xl text-gray-400 hover:text-gray-700" style={{ color: 'var(--muted)' }} onClick={() => { setShowUploadModal(false); setUploadError(null); setUploadFile(null); setCaption(''); setSelectedProduct(''); }} aria-label="Close">
                ×
              </button>
              ...
              <form onSubmit={async (e) => {
                ...
                <input id="reel-upload-file" ref={fileInputRef} type="file" accept="video/*" onChange={(e) => {
                setUploadError(null);
            ...
            }} className="file:mr-2 file:py-2 file:px-3 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 w-full border border-[var(--border)]" style={{ background: 'var(--bg-2)', color: 'var(--text)' }} disabled={uploading}/>
                ...
                <input id="reel-upload-caption" type="text" placeholder={t('reels.captionOptionalPlaceholder', 'Caption (optional)')} value={caption} onChange={e => setCaption(e.target.value)} className="border rounded px-3 py-2 text-base border-[var(--border)] focus:outline-none focus:border-purple-500" style={{ background: 'var(--bg-2)', color: 'var(--text)' }} disabled={uploading}/>
                ...
                <select id="reel-upload-product" className="border rounded px-3 py-2 text-base border-[var(--border)] focus:outline-none focus:border-purple-500" style={{ background: 'var(--bg-2)', color: 'var(--text)' }} value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)} disabled={uploading || products.length === 0}>
                  <option value="">{t('reels.noProduct', 'No Product (Just a Reel)')}</option>
                  {products.map(p => (<option key={p.id} value={p.id}>{p.title}</option>))}
                </select>
                {uploadError && <div className="text-red-600 text-sm">{uploadError}</div>}
                <button type="submit" className="bg-gradient-to-r from-purple-500 via-fuchsia-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold py-3 rounded shadow disabled:opacity-60 transition-all text-base mt-2" disabled={uploading || !uploadFile}>
      ...
      <div className="w-full max-w-md md:max-w-2xl flex flex-col gap-8">
        {initialLoading ? (<div className="w-full flex flex-col items-center">
            <ReelSkeleton />
            <ReelSkeleton />
            <ReelSkeleton />
          </div>) : fetchError ? (<div className="flex flex-col justify-center items-center h-96 gap-4">
            <span className="text-red-600 text-center">{fetchError}</span>
            <button className="px-4 py-2 rounded-lg bg-orange-600 text-white font-semibold disabled:opacity-60" onClick={() => { setFetchError(null); setRetryCount(c => c + 1); fetchReels(cursor, false); }}>
              {t('reels.retry', { defaultValue: 'Retry' })}
            </button>
          </div>) : reels.length === 0 ? (<div className="flex justify-center items-center h-96">
            <span className="text-[var(--muted)]">{t('reels.noReels', { defaultValue: 'No reels yet.' })}</span>
          </div>) : (<>
            {reels.map((reel, idx) => (<div key={reel.id + '-' + idx} className="rounded-xl shadow-lg bg-[var(--bg)] border border-[var(--border)] overflow-hidden flex flex-col">
                <div className="flex items-center gap-3 p-4">
                  {reel.profiles?.profile_image ? (<img src={reel.profiles.profile_image} alt={reel.profiles.name} className="w-10 h-10 rounded-full object-cover"/>) : (<User className="w-8 h-8 text-orange-600"/>)}
                  <div>
                ...
                <div className="w-full max-w-[350px] mx-auto aspect-[4/5] bg-black flex items-center justify-center rounded-lg">
                  <video src={reel.video_url} loop playsInline className="reel-video w-full h-full object-cover cursor-pointer rounded-lg" onClick={e => {
                ...
                }} onDoubleClick={() => handleLike(reel.id)} style={{ touchAction: 'manipulation' }}/>
                </div>
                <div className="p-4 flex flex-col gap-2">
                  <div className="text-[var(--text)] text-base font-medium">{reel.caption}</div>
                  <div className="flex items-center gap-6 mt-2">
                    <button className={`flex items-center gap-1 text-[var(--muted)] hover:text-orange-600`} onClick={() => handleLike(reel.id)}>
                    ...
                    <button className="flex items-center gap-1 text-[var(--muted)] hover:text-orange-600" onClick={() => {
                    ...
                    {reel.product_id && (<Link href={`/product/${reel.product_id}`} className="text-[var(--muted)] hover:text-orange-600 text-sm">
                  ...
                  {/* Comments Section */}
                  {openCommentReelId === reel.id && (<div className="mt-4">
                      <div className="flex flex-col gap-2">
                        {/* Existing comments */}
                        <div className="max-h-40 overflow-y-auto mb-2">
                          {comments[reel.id]?.length ? (comments[reel.id].map(cmt => (<div key={cmt.id} className="flex items-start gap-2 mb-2">
                                {cmt.profiles?.profile_image ? (<img src={cmt.profiles.profile_image} alt={cmt.profiles.name} className="w-7 h-7 rounded-full object-cover"/>) : (<User className="w-6 h-6 text-orange-600"/>)}
                                <div>
                              ...
                              </div>))) : (<span className="text-[var(--muted)] text-sm">{t('reels.noComments', { defaultValue: 'No comments yet.' })}</span>)}
                        </div>
                        {/* Comment input */}
                        ...
                        fetchComments(reel.id);
                          ...
                          <input type="text" className="flex-1 rounded-lg border border-[var(--border)] px-3 py-2 text-[var(--text)] bg-[var(--bg-2)] focus:outline-none focus:border-orange-600" placeholder={t('reels.addComment', { defaultValue: 'Add a comment...' })} value={commentInput} onChange={e => setCommentInput(e.target.value)} disabled={commentLoading}/>
                          <button type="submit" className="px-4 py-2 rounded-lg bg-orange-600 text-white font-semibold disabled:opacity-60" disabled={commentLoading || !commentInput.trim()}>
            ...
            <div ref={sentinelRef}/>
            {loading && !initialLoading && (<div className="w-full flex flex-col items-center py-4">
                <ReelSkeleton />
              </div>)}
            {!hasMore && reels.length > 0 && (<div className="flex justify-center items-center py-8">
