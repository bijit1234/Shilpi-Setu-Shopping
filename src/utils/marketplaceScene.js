...
function createSimpleAvatar(color = 0x3b82f6) {
    const group = new THREE.Group();
    // Taller body
    const body = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.95, 16), new THREE.MeshStandardMaterial({ color, roughness: 0.5 }));
    body.position.y = 0.475;
    body.userData.isAvatarPart = true;
    group.add(body);
    // Larger head
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.22, 16, 16), new THREE.MeshStandardMaterial({ color: 0xffe0bd, roughness: 0.4 }));
...
function createBillboard(url, w = 1.0, h = 1.25) {
    ...
    const tex = new THREE.TextureLoader().load(url, undefined, // onLoad
...
export async function initMarketplaceScene(canvas, stallsInput, dayMode = false) {
    const scene = new THREE.Scene();
    let cloudGroup = null;
    let cloudAnimOffsets = [];
    if (dayMode) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            for (let j = -1; j <= 1; j++) {
                ...
                const sphere = new THREE.Mesh(new THREE.SphereGeometry(radius, 32, 32), new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.15, transparent: false, opacity: 1 }));
    ...
    if (dayMode) {
        // Add sun
        const sun = new THREE.Mesh(new THREE.SphereGeometry(4, 32, 32), new THREE.MeshStandardMaterial({ color: 0xfff7b2, emissive: 0xffe066, emissiveIntensity: 0.7, roughness: 0.7 }));
            ...
            if (mesh && mesh.isMesh) {
                ...
                for (const m of mats) {
                    ...
                    if (m.map && typeof m.map.dispose === 'function')
                        m.map.dispose();
                    if (m.alphaMap && typeof m.alphaMap.dispose === 'function')
                        m.alphaMap.dispose();
                    if (m.emissiveMap && typeof m.emissiveMap.dispose === 'function')
                        m.emissiveMap.dispose();
                    if (m.normalMap && typeof m.normalMap.dispose === 'function')
                        m.normalMap.dispose();
                    if (m.roughnessMap && typeof m.roughnessMap.dispose === 'function')
                        m.roughnessMap.dispose();
                    if (m.metalnessMap && typeof m.metalnessMap.dispose === 'function')
    ...
    const buildStalls = async (nextInput) => {
        clearStalls();
        ...
        nextInput.forEach((stall, idx) => {
            ...
            // 5. STREET LIGHTS
            const makeLamp = (offsetX, offsetZ) => {
            ...
            sGroupCarousel.carouselUpdate = function updateBillboards() {
                ...
                pageProducts.forEach((p, i2) => {
                    ...
                    mesh.renderOrder = 2;
                    mesh.userData = { ...mesh.userData, productId: p.id };
                    sGroup.add(mesh);
                    billboards.push({ id: p.id, mesh });
                });
                if (leftArrow)
                    leftArrow.visible = sGroupCarousel.carouselPage > 0;
                if (rightArrow)
                    rightArrow.visible = sGroupCarousel.carouselPage < carouselPages.length - 1;
            };
            let leftArrow = null;
            let rightArrow = null;
            sGroupCarousel.carouselUpdate();
            sGroupCarousel.setFocused = (focused) => {
                if (leftArrow)
                    leftArrow.visible = focused && sGroupCarousel.carouselPage > 0;
                if (rightArrow)
                    rightArrow.visible = focused && sGroupCarousel.carouselPage < carouselPages.length - 1;
            };
            if (carouselPages.length > 1) {
                ...
                let arrowLabel = null;
                const makeArrowLabel = (text, x, y, z) => {
                    const canvas = document.createElement('canvas');
                    canvas.width = 900;
                    canvas.height = 60;
                    const ctx = canvas.getContext('2d');
                        ...
                        setTimeout(() => {
                            if (arrowLabel) {
                                ...
                                const fadeStep = () => {
                                    fade -= 0.08;
                                    if (arrowLabel)
                                        arrowLabel.material.opacity = Math.max(0, fade);
                                    if (fade > 0) {
                                        requestAnimationFrame(fadeStep);
                                    }
                                    else {
                                        if (arrowLabel && arrowLabel.parent)
                                            arrowLabel.parent.remove(arrowLabel);
                                        arrowLabel = null;
                                    }
                                };
                                fadeStep();
                ...
                if (leftArrow?.material && 'color' in leftArrow.material) {
                    leftArrow.material.color.set(0x3b82f6);
                }
                if (rightArrow?.material && 'color' in rightArrow.material) {
                    rightArrow.material.color.set(0x3b82f6);
                }
            }
            // 7. GO TO BUTTON
    ...
    if (markerSprite.material)
        markerSprite.material.depthTest = false;
    ...
    minimapOverlay.className = 'shilpisetu-minimap-overlay';
    ...
    updateMinimapSize();
    // --- THEME STRUCTURES ---
    function createStallStructureClassic(decor) {
        ...
        {
            const woodWall = new THREE.Mesh(new THREE.PlaneGeometry(6.6, 2.5), new THREE.MeshStandardMaterial({
                color: 0x8b5a2b,
                roughness: 0.8,
                metalness: 0.1,
                transparent: true,
        ...
        for (let i = -2; i <= 2; i += 2) {
            ...
            const wire = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.38, 8), new THREE.MeshStandardMaterial({ color: 0x5a4633 }));
            wire.position.y = 2.8;
            lantern.add(wire);
            const bulb = new THREE.Mesh(new THREE.SphereGeometry(0.09, 12, 12), new THREE.MeshStandardMaterial({ color: 0xffe4b5, emissive: 0xffe4b5, emissiveIntensity: 0.7 }));
        ...
        [-1, 1].forEach(i => {
            const shelf = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.08, 0.22), new THREE.MeshStandardMaterial({ color: 0x8b5a2b, roughness: 0.7 }));
            shelf.position.set(i * 2.2, 1.05, -1.18);
            group.add(shelf);
            const pot = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.19, 0.22, 10), new THREE.MeshStandardMaterial({ color: 0xc97a3d, roughness: 0.7 }));
            pot.position.set(i * 2.2, 1.18, -1.08);
            group.add(pot);
        });
        // Add clay pots
        for (let i = -2; i <= 2; i += 2) {
            const pot = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.28, 0.32, 12), new THREE.MeshStandardMaterial({ color: 0xc97a3d, roughness: 0.7 }));
        ...
        if (decor && decor.color) {
            group.traverse((obj) => {
                ...
                if (obj.isMesh && obj.material && obj.material.color) {
                    obj.material.color.set(decor.color);
                }
            });
        }
        return group;
    }
    function createStallStructureModern(decor) {
        ...
        const roof = new THREE.Mesh(new THREE.PlaneGeometry(7, 3.2), new THREE.MeshStandardMaterial({
            color: 0xffffff,
            metalness: 1,
            roughness: 0.18,
            transparent: true,
        ...
        const bgWall = new THREE.Mesh(new THREE.PlaneGeometry(6.6, 2.6), new THREE.MeshStandardMaterial({
            color: 0xffffff,
            metalness: 0.8,
            roughness: 0.18,
            transparent: true,
        ...
        const diyaBase = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.13, 0.07, 12), new THREE.MeshStandardMaterial({ color: 0xc9b037, metalness: 0.8, roughness: 0.3 }));
        diyaBase.position.set(-2.2, 0.32, 0.85);
        group.add(diyaBase);
        const diyaFlame = new THREE.Mesh(new THREE.SphereGeometry(0.03, 8, 8), new THREE.MeshStandardMaterial({ color: 0xffe066, emissive: 0xffe066, emissiveIntensity: 0.7 }));
        diyaFlame.position.set(-2.2, 0.39, 0.85);
        group.add(diyaFlame);
        // Decorative plate (thali) with pattern on right front
        const thali = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.02, 24), new THREE.MeshStandardMaterial({ color: 0xe0cda9, metalness: 0.7, roughness: 0.2 }));
        thali.position.set(2.2, 0.29, 0.85);
        group.add(thali);
        // Simple pattern: colored dots
        for (let k = 0; k < 6; k++) {
            const dot = new THREE.Mesh(new THREE.SphereGeometry(0.012, 8, 8), new THREE.MeshStandardMaterial({ color: [0xf43f5e, 0x3b82f6, 0xf59e42, 0x12b981, 0xfbbf24, 0x6366f1][k] }));
        ...
        for (let i = -2; i <= 2; i += 2) {
            ...
            const wire = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.38, 8), new THREE.MeshStandardMaterial({ color: 0x222222 }));
            wire.position.y = 3.0;
            pendant.add(wire);
            const bulb = new THREE.Mesh(new THREE.SphereGeometry(0.09, 12, 12), new THREE.MeshStandardMaterial({ color: 0xfff7b2, emissive: 0xffe066, emissiveIntensity: 0.8 }));
        ...
        const glassTable = new THREE.Mesh(new THREE.BoxGeometry(6, 0.32, 2.2), new THREE.MeshStandardMaterial({ color: 0xffffff, metalness: 0.9, roughness: 0.1, transparent: true, opacity: 0.45, emissive: 0x00fff7, emissiveIntensity: 0.08 }));
        ...
        [[-2.7, 1.1], [2.7, 1.1], [-2.7, -1.1], [2.7, -1.1]].forEach(([x, z]) => {
            const planter = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.18, 0.32), new THREE.MeshStandardMaterial({ color: 0x8fd19e, metalness: 0.5, roughness: 0.2 }));
            planter.position.set(x, 0.12, z);
            group.add(planter);
            const plant = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.22, 8), new THREE.MeshStandardMaterial({ color: 0x3b82f6, emissive: 0x00fff7, emissiveIntensity: 0.3 }));
    ...
    function createStallStructureFestive(decor) {
        ...
        for (let i = -3; i <= 3; i++) {
            ...
            const banner = new THREE.Mesh(new THREE.PlaneGeometry(0.7, 0.22), new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.18 }));
        ...
        for (let i = 0; i < 18; i++) {
            const confetti = new THREE.Mesh(new THREE.SphereGeometry(0.06, 6, 6), new THREE.MeshStandardMaterial({ color: confettiColors[i % confettiColors.length], emissive: confettiColors[i % confettiColors.length], emissiveIntensity: 0.22 }));
        ...
        for (let i = -2; i <= 2; i++) {
            ...
            const streamer = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.7, 8), new THREE.MeshStandardMaterial({ color: streamerColor, emissive: streamerColor, emissiveIntensity: 0.18 }));
    ...
    const animate = () => {
        ...
        if (minimap.enabled && minimap.camera) {
            ...
            if (minimap.marker.material)
                minimap.marker.material.rotation = angle;
    ...
    const dispose = () => {
        ...
        try {
            if (minimap.marker && minimap.marker.material) {
                const mat = minimap.marker.material;
                if (mat.map)
                    mat.map.dispose();
                mat.dispose();
            }
            ...
            try {
                ...
                if (typeof minimapRenderer.forceContextLoss === 'function') {
                    minimapRenderer.forceContextLoss();
                }
            }
            catch (e) { }
            try {
                if (minimapOverlay && minimapOverlay.parentElement)
                    minimapOverlay.parentElement.removeChild(minimapOverlay);
            }
            catch (e) { }
        }
        catch (e) { }
    ...
    const updateStalls = async (nextInput) => {
